...
export class TimeBlockService {
	...
	async create(dto: TimeBlockDto, userId: string) {
		const { name, color, duration, order } = dto

		// Убедитесь, что color всегда присутствует в объекте данных
		const data = {
			name,
			color: color || '', // Если color не определен, используйте пустую строку
			duration,
			order,
			user: {
				connect: {
